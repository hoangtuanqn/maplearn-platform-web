            ...
            <div className="t1-flex-center mt-6">
                ...
                >
                    <div className="t1-flex-center flex gap-1.5">
                        <button
                            className={`t1-flex-center flex size-9 cursor-pointer rounded-md duration-100 hover:bg-gray-100 active:bg-gray-100 ${page === 1 ? "pointer-events-none opacity-25" : ""}`}
                        ...
                        {documents &&
                            ...
                            Array.from({ length: documents.last_page }, (_, i) => i + 1).map((p) => (
                                <button
                                    key={p}
                                    className={`t1-flex-center text-md relative h-9 w-9 cursor-pointer rounded-md border duration-100 ${p === page ? "pointer-events-none border-blue-500 bg-white font-medium text-blue-400" : "border-transparent text-black opacity-80 hover:bg-gray-100 active:bg-gray-100"}`}
                                    onClick={() => setPage(p)}
                                    disabled={p === page}
                                >
                                    {p}
                        ...
                        <button
                            className={`flex h-9 w-9 cursor-pointer items-center justify-center rounded-md duration-100 hover:bg-gray-100 active:bg-gray-100 ${documents && page === documents.last_page ? "pointer-events-none opacity-25" : ""}`}
                            onClick={() => setPage(page + 1)}
